...
def migrate(migrator: Migrator, database: pw.Database, *, fake=False):
    ...
    with database.atomic():
        # First, handle the specific case of μετάξι (ID 142)
        # We know this is causing issues, so let's delete it directly
        migrator.sql("DELETE FROM sourcefilewordform WHERE wordform_id = 142")
        migrator.sql("DELETE FROM wordform WHERE id = 142")
        print("Deleted problematic wordform with ID 142")
