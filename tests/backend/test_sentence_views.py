...
def test_get_sentence_audio(client, test_sentence):
    ...
    response = client.get(
        f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/{test_sentence.id}/audio"
    )
    ...
    response = client.get(f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/999/audio")
...
def test_get_sentence_audio_wrong_language(client, test_sentence):
    ...
    response = client.get(f"/api/lang/sentence/fr/{test_sentence.id}/audio")
    assert response.status_code == 404
    assert b"Sentence not found" in response.data
...
def test_get_random_sentence(client, test_sentence):
    ...
    response = client.get(f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/random")
    assert response.status_code == 200
    data = response.json
    ...
    response = client.get(
        f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/random",
    ...
    response = client.get("/api/lang/sentence/fr/random")
    assert response.status_code == 404
    assert b"No matching sentences found" in response.data
...
def test_get_sentences_list(client, test_sentence):
    ...
    response = client.get(f"/lang/{TEST_LANGUAGE_CODE}/sentences")
    assert response.status_code == 200
...
def test_get_sentence_by_slug(client, test_sentence):
    ...
    response = client.get(f"/lang/{TEST_LANGUAGE_CODE}/sentence/{test_sentence.slug}")
    assert response.status_code == 200
    assert test_sentence.sentence.encode() in response.data

    # Test non-existent sentence
    response = client.get(f"/lang/{TEST_LANGUAGE_CODE}/sentence/nonexistent-slug")
    assert response.status_code == 404

    # Test wrong language code
    response = client.get(f"/lang/fr/sentence/{test_sentence.slug}")
    assert response.status_code == 404


def test_delete_sentence(client, test_sentence):
    """Test deleting a sentence."""
    response = client.delete(f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/{test_sentence.slug}")
    ...
    response = client.delete(f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/nonexistent-slug")
    assert response.status_code == 404
...
def test_rename_sentence(client, test_sentence):
    ...
    response = client.put(
        f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/{test_sentence.slug}/rename",
    ...
    # Test non-existent sentence
    response = client.put(
        f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/nonexistent-slug/rename",
        json={"new_text": new_text},
    )
    assert response.status_code == 404
...
def test_generate_sentence_audio(client, test_sentence):
    ...
    response = client.post(
        f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/{test_sentence.slug}/generate_audio"
    )
    assert response.status_code == 204

    # Test non-existent sentence
    response = client.post(
        f"/api/lang/sentence/{TEST_LANGUAGE_CODE}/nonexistent-slug/generate_audio"
    )
    assert response.status_code == 404
