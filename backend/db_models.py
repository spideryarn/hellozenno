...
class Sourcefile(BaseModel):
    ...
    # blobs average ~1.5MB, so they're excluded from Sourcefile.select_metadata()
    BLOB_FIELD_NAMES = ("image_data", "audio_data")

    @classmethod
    def blob_present(cls, field):
        """SQL for "this blob holds bytes", matching Python's bool() on the blob.

        A zero-byte upload is stored as b'' rather than NULL - validation only
        caps the maximum size - so IS NOT NULL would claim an empty file has
        playable audio. COALESCE keeps NULL out of the result, so callers can
        treat None as "column wasn't selected".
        """
        return fn.COALESCE(fn.LENGTH(field), 0) > 0

    @classmethod
    def select_metadata(cls, *extra):
        """Select every column except the image_data/audio_data blobs.

        Use this anywhere the blobs aren't served to the user - a bare select()
        pulls megabytes per row and throws them away. Rows come back with
        `has_image`/`has_audio` booleans in place of the blobs.
        """
        fields = [
            field
            for field in cls._meta.sorted_fields
            if field.name not in cls.BLOB_FIELD_NAMES
        ]
        return cls.select(
            *fields,
            cls.blob_present(cls.image_data).alias("has_image"),
            cls.blob_present(cls.audio_data).alias("has_audio"),
            *extra,
        )
