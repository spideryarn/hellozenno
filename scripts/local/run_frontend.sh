...
if ! npx supabase gen types typescript --local --schema public --schema auth > "$TMP_TYPES_FILE"; then
  echo "❌ ERROR: Docker Desktop app is not running and/or run \`supabase start\`" >&2
  rm -f "$TMP_TYPES_FILE"
  exit 1
fi
# The app's tables come from the backend's Peewee migrations, not Supabase migrations.
# Against an up-but-unmigrated local DB this command still exits 0 and emits an empty
# public schema ("Tables: { [_ in never]: never }"), which would clobber the committed
# file and collapse every table row type to `never`. Refuse to overwrite in that case.
if ! grep -q 'sourcedir: {' "$TMP_TYPES_FILE"; then
  echo "❌ ERROR: generated types have no public schema (no 'sourcedir' table)." >&2
  echo "   The local DB is up but unmigrated. Run ./scripts/local/migrate.sh first." >&2
  echo "   Left src/lib/database.types.ts untouched." >&2
  rm -f "$TMP_TYPES_FILE"
  exit 1
fi
# Prepend autogenerated header and write final file
...
*/
EOF
cat "$TMP_TYPES_FILE" >> src/lib/database.types.ts
rm -f "$TMP_TYPES_FILE"
