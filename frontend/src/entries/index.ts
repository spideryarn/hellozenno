...
const components = {
    minilemma: (target: HTMLElement, props: any) =>
        new MiniLemma({ target, props }),
    minisentence: (target: HTMLElement, props: any) =>
        new MiniSentence({ target, props }),
    miniwordform: (target: HTMLElement, props: any) =>
        new MiniWordform({ target, props }),
    miniwordformlist: (target: HTMLElement, props: any) =>
        new MiniWordformList({ target, props }),
    miniphrase: (target: HTMLElement, props: any) =>
        new MiniPhrase({ target, props }),
    sentence: (target: HTMLElement, props: any) =>
        new Sentence({ target, props }),
    flashcardapp: (target: HTMLElement, props: any) =>
        new FlashcardApp({ target, props }),
    flashcardlanding: (target: HTMLElement, props: any) =>
        new FlashcardLanding({ target, props }),
    auth: (target: HTMLElement, props: any) => new AuthPage({ target, props }),
    userstatus: (target: HTMLElement, props: any) =>
        new UserStatus({ target, props }),
};
